...
type maxQuantities struct {
	r, g, b int
}

type minQuantities struct {
	r, g, b int
}
...
func solve(input string) {
	...
	sumIds := 0
	// part 2
	minPerGame := make([]minQuantities, len(games))
	sumOfPowers := 0
	for i, game := range games {
		if isPossible(game, maxQuantities{r: 12, g: 13, b: 14}) {
		...
		minPerGame[i] = minQuantities{r: minR, g: minG, b: minB}
		sumOfPowers += minR * minG * minB
	}
	// part 1 output
...
func parseGame(line string) game {
	...
	rest := strings.TrimPrefix(line, fmt.Sprintf("Game %d: ", id))
	turnsText := strings.Split(rest, "; ")
	turns := make([]turn, len(turnsText))
	for i, turnText := range turnsText {
		turns[i] = parseTurn(turnText)
	}
...
func parseTurn(text string) turn {
	var r, g, b int
	numCols := strings.Split(text, ", ")
	for _, nc := range numCols {
		substrings := strings.Split(nc, " ")
		num, color := substrings[0], substrings[1]
...
func isPossible(g game, lim maxQuantities) bool {
	for _, turn := range g.turns {
		if turn.r > lim.r || turn.g > lim.g || turn.b > lim.b {
			return false
		}
	}
	return true
